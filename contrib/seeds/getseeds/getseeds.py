...
def confFile():
    ...
    if platform.system() == 'Windows':
        folder = os.path.join(os.path.join(os.environ['APPDATA'], '9TOKEN'))
    else:
        if platform.system() == 'Darwin':
            folder = os.path.expanduser('~/Library/Application Support/9TOKEN/')
        else:
            folder = os.path.expanduser('~/.9token')

    return os.path.join(folder, '9token.conf')

conf_path = confFile()
if not os.path.exists(conf_path):
    parser.error('''9TOKEN configuration file not found. Manually enter your RPC password.\r\n'''
        ...
        '''rpcpassword=%x\r\n'''
        '''\r\n'''
        '''Keep that password secret! After creating the file, restart 9TOKEN.''' % (conf_path, random.randrange(2**128)))
...
for node in json.loads(data.decode()):
    ...
    try:
        ...
        if e.errno == errno.ECONNREFUSED:
            print ('Unable to communicate with 9TOKEN RPC')
        break
